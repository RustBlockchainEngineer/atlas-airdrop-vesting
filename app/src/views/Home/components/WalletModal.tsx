...
function WalletModal({ visible, setVisible }) {
  ...
  const { colors } = theme;
  const [searched, setSearched] = useState(wallets);
              ...
              <Box maxHeight="200px" overflowX="hidden">
                {searched.map((wallet, index) => (
                  <Flex
                    ...
                    onClick={async (e) => {
                      // const indexForSelectedWallet = wallets.findIndex(
                      //   (walletObj) => walletObj.url === wallet.url,
                      // );
                      // await connectWallet(indexForSelectedWallet, type, chainId);
                      // setVisible(false);
                    }}
                  >
                    <img
