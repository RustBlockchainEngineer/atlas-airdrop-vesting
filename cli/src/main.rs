...
use atlas_vesting_v1::accounts as vesting_accounts;
use atlas_vesting_v1::instruction as vesting_instruction;
use atlas_vesting_v1::GlobalState;
use atlas_vesting_v1::Vesting;
// The `accounts` and `instructions` modules are generated by the framework.
use clap::Parser;
use rand::rngs::OsRng;
...
fn main() -> Result<()> {
    println!("Starting atlas airdrop vesting cli ...");
    let opts = Opts::parse();

    // Wallet and cluster params.
    let payer = read_keypair_file(&*shellexpand::tilde("./id.json"))
        .expect("cli requires a keypair file");
...
fn create_global_state(client: &Client, pid: Pubkey) -> Result<()> {
    // Program client.
...
//
// Make sure to run a localnet with the program deploy to run this example.
fn create_vesting(client: &Client, pid: Pubkey) -> Result<()> {
    let program = client.program(pid);
...
fn update_vesting(client: &Client, pid: Pubkey) -> Result<()> {
    let program = client.program(pid);
...
pub fn deposit_vesting(client: &Client, pid: Pubkey) -> Result<()> {
    let program = client.program(pid);
    let authority = program.payer();
...
pub fn withdraw_vesting(client: &Client, pid: Pubkey) -> Result<()> {
    let program = client.program(pid);
    let authority = program.payer();
...
pub fn claim(client: &Client, pid: Pubkey) -> Result<()> {
    let program = client.program(pid);
    let authority = program.payer();
